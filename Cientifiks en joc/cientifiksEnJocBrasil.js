document.addEventListener("DOMContentLoaded", function() {
  ...
  const playerSize = 50;
  const fruitSize = 30;
  const initialPlayerPos = [gameContainer.clientWidth / 2 - playerSize / 2, gameContainer.clientHeight - playerSize - 10];
  const initialFruitPos = [Math.random() * (gameContainer.clientWidth - fruitSize), 0];
  const fruitSpeed = 5;
  ...
  function draw() {
    ...
    if (
      ...
      score++;
      fruitPos = [Math.random() * (gameContainer.clientWidth - fruitSize), 0];
    }
  ...
  document.addEventListener("keydown", function(e) {
      ...
      } else if (e.key === "ArrowRight" && playerPos[0] < gameContainer.clientWidth - playerSize) {
        playerPos[0] += movementSpeed;
      }
  });
